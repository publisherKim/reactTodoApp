...
### 정리
```
  프로젝트에서 필요한 대부분의 주요 인터페이스를 완성
  다른 라이브러리들도 응용 가능
  컴포넌트 라이프 사이클 메서드에서 각 라이브러리의 함수를 호출
  리액트 형태로 만든 라이브러리를 사용하는 것이 편할 수도 있지만,
  세부적으로 설정하거나 컴포넌트를 커스터마이징하는 작업은 매우 복잡해 질수 있다.
  (실력에따라 잘 판단해서 쓰자. 어설프게 할거면 건드리지 말자)

  특정 자바스크립트 라이브러리가 있다면 공식적이 아니면 항상 주의해라
  업데이트가 중단 될수도 있다.
  추가로 프로젝트의 마지막 수정 날짜는 언제고 유지보수는 잘하는 지를 점검 하는게 좋다.

  공식적으로 만든 리액트 라이브러리가 아니라면 대부분은 원본 라이브러리를 설치하여 사용하면 좋다.

  앞으로 서버 API 요청을 통해 필요한 데이터를 등록하고, 등록한 데이터를 조회하는 방법을 구현하자
```
