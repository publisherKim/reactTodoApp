...
```
  ...
  리듀서: 상태를 변화시키는 로직이 있는 함수 cf: https://www.vobour.com/%EB%A6%AC%EB%8D%95%EC%8A%A4-redux-%EC%9D%98-%EB%A6%AC%EB%93%80%EC%84%9C-reducer-%EA%B0%80-%EC%88%9C%EC%88%98-%ED%95%A8%EC%88%98%EC%97%AC%EC%95%BC%EB%A7%8C-%ED%95%98%EB%8A%94-%EC%9D%B4%EC%9C%A0
...
```
  1. 스토어는 단한개
    - 스토어를 복수개를 사용할수도 있으나 관리하기 어려워지므로 스토어는 한개를 유지하고 리듀서를 복수로 만들어서 처리하는 것이 좋다.
  2. state는 읽기 전용
    상태를 절대로 강제로 수정하지 말자.
    dispatch를 통해서 값을 변경한다.(java의 get, set을 생각해보자)
  3. 변화는 순수함수로 구성
    모든 변화는 순수 함수로 구성해야 한다.
    순수함수에서 결과 값을 출력할 때는 파라미터 값에만 의존해야 한다.
    같은 파라미터는 언제나 같은 결과를 출력해야만 한다.
```

### 정리
```
  1. 리덕스는 더욱 효율적으로 상태 관리를 할 수 있는 라이브러리이다.
  2. 스토어에 상태 정보를 가진 객체를 넣어둔다.
  3. 액션이 디스패치 되었을때 리듀서 함수를 이용하여 상태를 변화시키는것이 주요 역활이다.
  4. 상태가 변화될 때마다 스토어에 구독된 함수를 실행한다.
```
